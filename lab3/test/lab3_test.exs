defmodule Lab3Test do
  ...
  test "messages to user", %{room: room} do
    room = Labb3.join(room, "Joe")
    room = Labb3.send_message(room, "Robert", "Joe", "Hello Joe")
    assert Labb3.messages_to_user(room, "Joe") == ["Hello Joe"]
    assert Labb3.messages_to_user(room, "Robert") == []
  end

  test "messages from user", %{room: room} do
    room = Labb3.join(room, "Joe")
    room = Labb3.send_message(room, "Robert", "Joe", "Hello Joe")
    assert Labb3.messages_from_user(room, "Robert") == ["Hello Joe"]
    assert Labb3.messages_from_user(room, "Joe") == []
  end
end
